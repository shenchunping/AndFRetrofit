...
public final class HttpLoggingInterceptor implements Interceptor {
    ...
    public Response intercept(Chain chain) throws IOException {
        ...
        logBuilder.append("--> " + request.method() + ' ' + request.url() + ' ' + protocol);
        ...
        if (logHeaders) {
            if (hasRequestBody) {
                ...
                if (requestBody.contentType() != null) {
                    logBuilder.append("\n");
                    logBuilder.append("Content-Type: " + requestBody.contentType());
                }
                if (requestBody.contentLength() != -1) {
                    logBuilder.append("\n");
                    logBuilder.append("Content-Length: " + requestBody.contentLength());
                }
            }
            ...
            for (int i = 0, count = headers.size(); i < count; i++) {
                ...
                if (!"Content-Type".equalsIgnoreCase(name) && !"Content-Length".equalsIgnoreCase(name)) {
                    logBuilder.append("\n");
                    logBuilder.append(name + ": " + headers.value(i));
                }
            }
        ...
        try {
            ...
            logBuilder.append("\n");
            logBuilder.append("<-- HTTP FAILED: " + e);
            ...
            if (!logBody || !HttpHeaders.hasBody(response)) {
                ...
                if (contentType != null) {
                    try {
                        ...
                        logBuilder.append("\n");
                        logBuilder.append("Couldn't decode the response body; charset is likely malformed.");
                        logBuilder.append("\n");
                        logBuilder.append("<-- END HTTP");
                        Logger.i(logBuilder.toString());
                        return response;
                ...
                if (contentLength != 0) {
                    ...
                    logBuilder.append("");
                    logBuilder.append("\n");
                    logBuilder.append(buffer.clone().readString(charset));
                }
